require_relative '../../../lib/onestop_id_registry/entities/feed'

describe OnestopIdRegistry::Entities::Feed do
  context 'all' do
  end
  ...
  context 'individual' do
    ...
    it 'will cleanly fail when no Onestop ID or JSON blob provided' do
      expect {
        OnestopIdRegistry::Entities::Feed.new()
      }.to raise_error(ArgumentError)
    end

    it 'will cleanly fail when no JSON file found to go with provided Onestop ID' do
      expect {
        OnestopIdRegistry::Entities::Feed.new(onestop_id: 'f-9q9-NoBART')
